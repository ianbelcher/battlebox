...
func _init() -> void:
	...
	failures += _expect(McaWorld.map_block("minecraft:birch_planks") == Blocks.PLANKS, "birch_planks")
	failures += _expect(McaWorld.map_block("minecraft:red_wool") == Blocks.WOOL_RED, "red_wool")
	failures += _expect(McaWorld.map_block("minecraft:blue_stained_glass") == Blocks.GLASS, "stained glass")
