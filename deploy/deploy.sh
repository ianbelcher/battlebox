#!/usr/bin/env bash
#
# Put a specific build live. Runs ON the droplet; CI invokes it over ssh:
#
#     echo "$GHCR_TOKEN" | ssh deploy@battlebox.games \
#       /srv/battlebox/deploy.sh <image-tag>
#
# The registry token arrives on stdin rather than in the arguments, so it
# never appears in the droplet's process list or in bash history.
#
...
IMAGE="ghcr.io/ianbelcher/battlebox:${TAG}"
...
if [ ! -t 0 ]; then
  ...
  if [ -n "$TOKEN" ]; then
    ...
    printf '%s' "$TOKEN" | docker login ghcr.io -u ianbelcher --password-stdin
  fi
fi
...
echo "==> pulling"
docker compose pull --quiet server caddy
...
got="$(curl -fsS --max-time 5 http://127.0.0.1:8081/downloads/version.txt | tr -d '[:space:]')"
if [ "$want" != "$got" ]; then
...
if [ "$code" != "101" ]; then
  ...
  docker compose logs --tail 60 server
  exit 1
fi
